...
function SimulatorContent() {
  ...
  const [prediction, setPrediction] = useState<PredictionResult>(() => predictMatch(homeTeam, awayTeam, true));
  ...
  // Recalculate whenever teams change
  useEffect(() => {
    setPrediction(predictMatch(homeTeam, awayTeam, true));
  ...
  return (
    <div className="space-y-10 pb-12">
      ...
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 p-6 sm:p-8 rounded-3xl border border-slate-800 shadow-xl text-white flex flex-col sm:flex-row sm:items-center justify-between gap-4 animate-fadeIn">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/15 border border-blue-400/30 text-blue-300 text-xs font-mono font-bold mb-3">
            <Cpu className="w-3.5 h-3.5 shrink-0 text-blue-400" />
        ...
        <div className="flex items-center gap-2 bg-white/10 px-4 py-2 rounded-2xl border border-white/15 text-xs font-mono text-slate-200 shrink-0 shadow-sm">
          <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
          <span>100% Neutral World Cup Venue</span>
        </div>
      </div>
